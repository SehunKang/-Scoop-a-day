...
## 회고
프로그래밍이 음악이라면 서비스는 오페라구나... 라는 생각이 들었다. 기획부터 개발, 출시까지의 전반적인 과정을 굉장히 간소하게나마 경험해보면서(사실 개발의 모든 과정을 경험했다고 할 수 없다) 값진 경험을 했다고
생각한다.  
나만의 애플리케이션을 출시하면서 소프트웨어가 가진 마법에 더욱 깊이 빠질 수 있었고 앱 개발자가 되기로 목표를 정한것에 다시한번 확신이 드는 시간이었다.
나중에 취직을 하더라도 지속적으로 나만의 애플리케이션을 만들고 관리하면서 내가 배운 기술과 인사이트를 지속적으로 시장에서 실험하고 성과를 낼 수 있을 것 같다. 이렇게 할 수 있는 직업이 또 있을까?  
프로젝트를 진행하면서 개인적으로 힘들었던 점은 기획단계에서 내가 어떤일을 얼마만큼의 시간 안에 할 수 있을까 계획하는 일과 디자인을 구상하고 에셋을 구하는 일이었다. 코드는 어떻게든 학습해서 구현할 수 있지만
상기한 두 케이스는 감으로, 내 멋대로 해결할 수 밖에 없었던 것 같다.
